...
setup() {
  ...
  const clickSignup = async function () {
    if (!state.form.email.status){
      ...
      await store.commit('accountStore/setAlertMessage', '이메일 중복 확인이 필요합니다.')
  ...
  const checkEmail = async function () {
    ...
    if (result.data.data) {
      ...
      await store.commit('accountStore/setAlertMessage', '사용 가능한 이메일입니다.')
      ...
      state.alert = false
      await store.commit('accountStore/setAlertColor', 'error')
      await store.commit('accountStore/setAlertMessage', '이미 사용 중인 이메일입니다.')
      await store.commit('accountStore/setAlertIcon', 'alert')
      state.alert = true
    }
  }
  ...
  const checkNickname = async function () {
    ...
    if (result.data.data) {
      ...
      state.alert = false
      await store.commit('accountStore/setAlertColor', 'error')
      await store.commit('accountStore/setAlertMessage', '이미 사용 중인 닉네임입니다.')
      await store.commit('accountStore/setAlertIcon', 'alert')
      state.alert = true
    }
  }
